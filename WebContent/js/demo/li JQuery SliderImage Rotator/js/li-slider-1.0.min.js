(function ($) {
    ...
    function Slider($newSlider, settings) {
        ...
        this.init = function () {
            ...
            initAttributes();
            if (settings.active_links) initLinks();
            initStructure();
            initAnimation();
            if (settings.auto_play) playSlider()
        };
        var inProgres = function (fun) {
            if (inTransition) {} else fun()
        };
        var inProgresSpec = function (pos, time) {
            if (inTransition) {} else {
        ...
        var initActiveButtons = function () {
            if (settings.auto_hide) {
                ...
                $slider.not(activeButtons).mouseenter(function () {
                    ...
                    activeButtons.delay(settings.buttons_hide_time).fadeOut(settings.buttons_hide_delay)
                });
                $slider.not(activeButtons).mouseenter(function () {
                    activeButtons.stop(true, true).delay(settings.buttons_show_delay).fadeIn(settings.buttons_show_time)
                }).mouseleave(function () {
        ...
        var initLinks = function () {
            $slider.mouseup(function (event) {
                if ($(event.target).parent().is('div.li-banner-image-wrap')) {
                    var info_link = $($sliderImage[oSlider.currentImage]).find('>a').attr('href');
                    if (info_link) document.location.href = info_link
                }
            })
        };
        var initAttributes = function () {
            ...
            for (image; image < $sliderImage.size(); image++) {
                ...
                for (i = 0; i < custClasses.length; i++) {
                    if (custClasses[i].indexOf('transition_') > -1) transition = custClasses[i].replace('transition_', '');
                    if (custClasses[i].indexOf('delay_') > -1) delay = custClasses[i].replace('delay_', '');
                    if (custClasses[i].indexOf('time_') > -1) time = parseInt(custClasses[i].replace('time_', ''))
                }
                $($sliderImage[image]).data({
                    'delay': delay,
        ...
        var initStructure = function () {
            container = $('<div class="li-banner-image-wrap" ></div>').css({
                'position': 'absolute',
                'top': '0px',
                'left': '0px',
                'overflow': 'hidden',
                ...
                if (animations[settings.animation]) {
                    ...
                    if (animations[settings.animation][0] >= animations['Regular-Custom'][0]) {
                        for (var i = 0; i < animList.length; i++) {
                            if (animations[animList[i]]) animStructure[animations[animList[i]][2]]++;
                            else animList[i] = 'None'
                        }
                    }
                } else settings.animation = 'None';
                if (animStructure[1] > 0) init_2_X_Sections();
                if (animStructure[2] > 0) initBgSections();
                if (animStructure[3] > 0) initSections();
                if (animStructure[4] > 0) initVertSections();
                if (animStructure[5] > 0) initSqrSections()
            }
        };
        var initSections = function () {
            var sqr_sections, sqr_section, image, sectionsX = 2,
                sectionsY = 2,
                sectionsZ = 2;
            ...
            for (var y = 0; y < sectionsY; y++) {
                for (var i = 0; i < sectionsX; i++) {
                    for (var z = 0; z < sectionsZ; z++) {
                        sqr_section = $('<div class="li-sectors' + z + '" id="' + oSlider.prefix + '-sqr-sect-' + y + '-' + i + '-' + z + '" style="left:' + i * width + 'px; top: ' + y * height + 'px; width:' + width + 'px; height:' + height + 'px; position: absolute; opacity: 0; overflow: hidden;">');
        ...
        var init_2_X_Sections = function () {
            var sections_2, section_2, image, sectionsX = 2,
                sectionsY = 1,
                sectionsZ = 2;
            var url = $('img', $sliderImage[oSlider.currentImage]).attr('src');
            var xPos, tOffset;
            ...
            for (var y = 0; y < sectionsY; y++) {
                for (var i = 0; i < sectionsX; i++) {
                    for (var z = 0; z < sectionsZ; z++) {
                        section_2 = $('<div class="li-sectors_2_X-' + z + '" id="' + oSlider.prefix + '-sect-2-x-' + y + '-' + i + '-' + z + '" style="left:' + i * width + 'px; top: ' + y * height + 'px; width:' + width + 'px; height:' + height + 'px; position: absolute; opacity: 0; overflow: hidden;">');
                        image = '<img src="" alt="Pic ' + y + '.' + i + '" style="position: absolute; left: -' + i * width + 'px; top: -' + y * height + 'px" />';
        ...
        var initBgSections = function () {
            ...
            for (var y = 0; y < sectionsY; y++) {
                for (var i = 0; i < sectionsX; i++) {
                    section = '<div class="sectors-bg" id="' + oSlider.prefix + '-bg-sect-' + y + '-' + i + '" style="left:' + i * width + 'px; top: ' + y * height + 'px; width:' + width + 'px; height:' + height + 'px; position: absolute; opacity: 0" >';
                    sections = $(sections).add($(section).css({
                        background: bgColor + " url('" + url + "') no-repeat ",
                        backgroundPosition: -i * width + "px " + -y * height + "px ",
                        "z-index": -100
                    }))
                }
            }
        ...
        var initVertSections = function () {
            ...
            for (i = 0; i < sectionsXRed; i++) {
                section = '<div class="v-sectors" id="' + oSlider.prefix + '-v' + i + '" style="left:' + i * width + 'px; top: 0px; width:' + width + 'px; height:' + height + 'px; position: absolute; opacity: 0" >';
                sections = $(sections).add($(section).css({
                    background: bgColor + " url('" + url + "') no-repeat ",
        ...
        var initSqrSections = function () {
            var section, sectionsY = settings.sqr_sections_Y,
                sectionsX;
            var bgColor = $slider.css("background-color");
            var url = $('img', $sliderImage[oSlider.currentImage]).attr('src');
            ...
            for (y = 0; y < sectionsYRed; y++) {
                for (i = 0; i < sectionsX; i++) {
                    section = '<div id="' + oSlider.prefix + '-sqr-' + i + '-' + y + '" class="sqr-sectors" style="left:' + i * width + 'px; top: ' + y * height + 'px; width:' + width + 'px; height:' + height + 'px; position: absolute; opacity: 0" >';
                    sections = $(sections).add($(section).css({
                        background: bgColor + " url('" + url + "') no-repeat ",
                        backgroundPosition: -i * width + "px " + -y * height + "px ",
                        "z-index": -100
        ...
        var showNextImage = function () {
            ...
            var time = $($sliderImage[pos]).data('time');
            if (!time) time = settings.trans_period;
            if (transition) showImage_random(pos, time, '', transition);
        ...
        var playEffect = function () {
            $('img', $sliderImage[oSlider.currentImage]).css({
                'width': settings.width,
                'height': settings.height,
                'left': 0,
                'position': 'absolute'
                ...
                $('img', $sliderImage[prevPosition]).css({
                    'width': '',
                    'height': '',
                    'left': '',
                    'position': ''
        ...
        var playSlider = function () {
            ...
            if (isNaN(currDelay) || currDelay == '') currDelay = settings.delay;
        ...
        this.clearLi = function () {
            $($sliderImage[oSlider.prevPosition]).css({
                'z-index': '',
                'left': '',
                'top': '',
                'width': '',
            ...
            $($sliderImage[oSlider.currentImage]).css({
                'z-index': '',
                'left': '',
                'top': '',
                'width': '',
        ...
        this.reInit_2_X_Sectors = function () {
            $('.li-sectors_2_X-0', $slider).css({
                'z-index': '',
                'opacity': 0
            });
            $('.li-sectors_2_X-1', $slider).css({
                'z-index': '',
                'opacity': 0
            });
            $('#' + oSlider.prefix + '-sect-2-x-0-0-0').find('img').css({
                'left': 0,
                'top': 0,
                'width': '',
                'height': ''
            });
            $('#' + oSlider.prefix + '-sect-2-x-0-0-1').find('img').css({
                'left': 0,
                'top': 0,
                'width': '',
                'height': ''
            });
            $('#' + oSlider.prefix + '-sect-2-x-0-1-0').find('img').css({
                'left': '-' + oSlider.width2 / 2 + 'px',
                'top': 0,
                'width': '',
                'height': ''
            });
            $('#' + oSlider.prefix + '-sect-2-x-0-1-1').find('img').css({
                'left': '-' + oSlider.width2 / 2 + 'px',
                'top': 0,
                'width': '',
                'height': ''
        ...
        this.reInitSectors = function () {
            $('.li-sectors0', $slider).css({
                'z-index': '',
                'opacity': 0
            });
            $('.li-sectors1', $slider).css({
                'z-index': '',
                'opacity': 0
            });
            $('#' + oSlider.prefix + '-sqr-sect-0-0-0').find('img').css({
                'left': 0,
                'top': 0
            });
            $('#' + oSlider.prefix + '-sqr-sect-0-0-1').find('img').css({
                'left': 0,
                'top': 0
            });
            $('#' + oSlider.prefix + '-sqr-sect-0-1-0').find('img').css({
                'left': '-' + oSlider.width4 / 2 + 'px',
                'top': 0
            });
            $('#' + oSlider.prefix + '-sqr-sect-0-1-1').find('img').css({
                'left': '-' + oSlider.width4 / 2 + 'px',
                'top': 0
            });
            $('#' + oSlider.prefix + '-sqr-sect-1-0-0').find('img').css({
                'left': 0,
                'top': '-' + oSlider.height4 / 2 + 'px'
            });
            $('#' + oSlider.prefix + '-sqr-sect-1-0-1').find('img').css({
                'left': 0,
                'top': '-' + oSlider.height4 / 2 + 'px'
            });
            $('#' + oSlider.prefix + '-sqr-sect-1-1-0').find('img').css({
                'left': '-' + oSlider.width4 / 2 + 'px',
                'top': '-' + oSlider.height4 / 2 + 'px'
            });
            $('#' + oSlider.prefix + '-sqr-sect-1-1-1').find('img').css({
                'left': '-' + oSlider.width4 / 2 + 'px',
                'top': '-' + oSlider.height4 / 2 + 'px'
            })
        };
