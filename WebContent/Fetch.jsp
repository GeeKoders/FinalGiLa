...
ResultSet rs=data.getQuery("select 套裝行程檔.行程流水號,景點檔.景點代號,會員帳號,行程主題名稱,景點檔.景點代號,景點名稱,檔名 from 套裝行程檔 inner join 行程明細檔 on 套裝行程檔.行程流水號=行程明細檔.行程流水號 inner join 景點檔 on 行程明細檔.景點代號=景點檔.景點代號 inner join 景點照片檔 on 景點檔.景點代號=景點照片檔.景點代號 where 會員帳號='Bob@gmail.com' and 套裝行程檔.行程流水號=10005 and 序號=1 order by 套裝行程檔.行程流水號 desc") ; 
String str=request.getParameter("cust_plan") ;
out.println(str) ; 
%>
